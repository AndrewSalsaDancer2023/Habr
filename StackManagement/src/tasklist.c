...
const float stack_usage_dfefault = -1.0f;
...
struct task* init_task(void (*func)(void *), void *arg)
{
	...
	task->arg = arg;
	task->id = id++;
	// task->stat = init_statistick(task->id, stack_usage_dfefault); 
...
void delete_task_list_item(struct task_list_item** item)
{
	...
	deallocate_fixedsize_stack(&(*item)->task->stack);
	free((*item)->task);
	free(*item);
}
